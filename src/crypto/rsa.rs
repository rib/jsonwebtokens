...
pub fn sign(algorithm: AlgorithmID, secret_or_key: &SecretOrKey, message: &str) -> Result<String, Error> {
    ...
    match secret_or_key {
        ...
        _ => Err(Error::InvalidInput(ErrorDetails::new("Missing RSA private key for signing")))
    }
}
...
pub fn verify(algorithm: AlgorithmID, secret_or_key: &SecretOrKey, message: &str, signature: &str) -> Result<(), Error> {
    match secret_or_key {
        ...
        SecretOrKey::RsaParameters(n, e) => {
            ...
            let signature_bytes = b64_decode(signature)?;
            pubkey.verify(rsa_params, message.as_ref(), &signature_bytes)
                .map_err(|_| Error::InvalidSignature())
        },
        _ => Err(Error::InvalidInput(ErrorDetails::new("Missing RSA public key for verifying")))
    }
}